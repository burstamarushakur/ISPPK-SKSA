...
export default function TeacherForm() {
  ...
  const canNext=()=>{if(!instrument)return false;if(step===0)return !validateStep0();if(step===1)return instrument.teacherRubric.every(i=>teacherScores[i.id]>=1&&teacherScores[i.id]<=5);if(step===2)return instrument.studentRubric.every(i=>studentScores[i.id]>=1&&studentScores[i.id]<=5);return reflection1.trim().length>0&&reflection2.trim().length>0}
  const next=()=>{setError('');const v=step===0?validateStep0():'';if(v){setError(v);return}if(!canNext()){setError('Sila lengkapkan bahagian ini sebelum meneruskan.');return}setStep(s=>Math.min(3,s+1));window.scrollTo({top:0,behavior:'smooth'})}
  ...
  const submit=async()=>{
    ...
    try{
      const now=new Date().toISOString();const obs:Observation={
        ...
        selfTeacherScores:teacherScores,selfStudentScores:studentScores,finalTeacherScores:{},finalStudentScores:{},reflection1:reflection1.trim(),reflection2:reflection2.trim(),observerSummary:observerSummary.trim(),status:'submitted',googleFormStatus:'pending',createdAt:now,updatedAt:now
  ...
  return <main className="container">
    ...
    <div className="steps">{['Bahagian A-D · Maklumat','Borang Skor Guru','Borang Skor Murid','Skor · Refleksi · Rumusan'].map((s,i)=><span key={s} className={`step-pill ${step===i?'active':''}`}>{i+1}. {s}</span>)}</div>
    ...
    {step===0&&<div className="card">
      <div className="official-info-block"><h2>Bahagian A - Maklumat Pemantau</h2><div className="form-grid">
        <div className="field span-2"><label>1. Nama Pemantau *</label><SearchSelect value={evaluatorId} onChange={setEvaluatorId} options={evaluators.filter(e=>e.active).sort((a,b)=>a.sortOrder-b.sortOrder).map(e=>({value:e.id,label:e.name}))} placeholder="Cari nama pemantau..."/><div className="helper">Senarai diambil daripada kolum PEGAWAI PENILAI (P1 & P2). Pilihan ini tidak dikunci mengikut nama PYD.</div></div>
      ...
      <div className="official-info-block"><h2>Bahagian B - Maklumat Sekolah</h2><div className="form-grid">
        <div className="field"><label>1. Nama Sekolah</label><input className="input official-readonly" readOnly value={school?.schoolName||''}/></div>
        <div className="field"><label>2. Alamat Sekolah</label><input className="input official-readonly" readOnly value={school?.address||''}/></div>
        <div className="field"><label>3. No. Tel.</label><input className="input official-readonly" readOnly value={school?.phone||''}/></div>
        <div className="field"><label>4. No. Faks</label><input className="input official-readonly" readOnly value={school?.fax||''}/></div>
        <div className="field span-2"><label>5. E-mel</label><input className="input official-readonly" readOnly value={school?.officialEmail||''}/></div>
        <div className="field"><label>6. Kod Sekolah</label><input className="input official-readonly" readOnly value={school?.schoolCode||''}/></div>
        <div className="field"><label>7. Gred Sekolah</label><input className="input official-readonly" readOnly value={school?.grade||''}/></div>
        <div className="field"><label>8. Jenis Sekolah</label><input className="input official-readonly" readOnly value={school?.schoolType||''}/></div>
        <div className="field"><label>9. Lokasi Sekolah</label><input className="input official-readonly" readOnly value={school?.location||''}/></div>
        <div className="field"><label>10. PPD</label><input className="input official-readonly" readOnly value={school?.ppd||''}/></div>
        <div className="field"><label>11. Negeri</label><input className="input official-readonly" readOnly value={school?.state||''}/></div>
        <div className="field span-2"><label>12. Program Sekolah</label><input className="input official-readonly" readOnly value={school?.schoolProgram||''}/></div>
      </div></div>
      ...
      <div className="official-info-block"><h2>Bahagian C - Maklumat Guru Yang Dicerap</h2><div className="form-grid">
        ...
        <div className="field"><label>3. No. Tel. *</label><input className="input" value={teacherPhone} onChange={e=>setTeacherPhone(e.target.value)}/></div>
        <div className="field span-2"><label>4. E-mel *</label><input className="input" type="email" value={teacherEmail} onChange={e=>setTeacherEmail(e.target.value)}/></div>
        <div className="field span-2"><label>5. Kelulusan Akademik Tertinggi *</label><input className="input" value={academicQualification} onChange={e=>setAcademicQualification(upper(e.target.value))}/></div>
        <div className="field"><label>6. Kelulusan Ikhtisas *</label><input className="input" value={professionalQualification} onChange={e=>setProfessionalQualification(upper(e.target.value))}/></div>
        <div className="field"><label>7. Opsyen *</label><input className="input" value={optionName} onChange={e=>setOptionName(upper(e.target.value))}/></div>
        <div className="field"><label>8. Pengalaman Mengajar (tahun) *</label><input className="input" type="number" min="0" value={teachingExperienceYears??''} onChange={e=>setTeachingExperienceYears(e.target.value===''?null:Number(e.target.value))}/></div>
        <div className="field"><label>9. Mata Pelajaran Yang Dicerap *</label><SearchSelect value={subjectId} onChange={setSubjectId} options={subjects.filter(s=>s.active).map(s=>({value:s.id,label:s.name}))}/></div>
        <div className="field"><label>10. Pengalaman Mengajar Mata Pelajaran (tahun) *</label><input className="input" type="number" min="0" value={subjectTeachingExperienceYears??''} onChange={e=>setSubjectTeachingExperienceYears(e.target.value===''?null:Number(e.target.value))}/></div>
        <div className="field span-2"><label>11. Jawatan Khas</label><input className="input" value={specialPosition} onChange={e=>setSpecialPosition(upper(e.target.value))} placeholder="Contoh: GURU CEMERLANG / TIADA"/></div>
      </div></div>

      <div className="official-info-block"><h2>Bahagian D - Maklumat Kelas Yang Dicerap</h2><div className="form-grid">
        <div className="field"><label>12. Bil. Murid *</label><input className="input" type="number" min="1" value={studentsPresent??''} onChange={e=>setStudentsPresent(e.target.value===''?null:Number(e.target.value))}/></div>
        <div className="field"><label>Lelaki *</label><input className="input" type="number" min="0" value={studentsMale??''} onChange={e=>setStudentsMale(e.target.value===''?null:Number(e.target.value))}/></div>
        <div className="field"><label>Perempuan *</label><input className="input" type="number" min="0" value={studentsFemale??''} onChange={e=>setStudentsFemale(e.target.value===''?null:Number(e.target.value))}/></div>
        <div className="field span-2"><label>13. Tahun/Tingkatan *</label><SearchSelect value={classId} onChange={setClassId} options={classes.filter(c=>c.active).map(c=>({value:c.id,label:`TAHUN ${c.year} - ${c.name}`,group:`Tahun ${c.year}`}))}/></div>
        <div className="field span-2"><label>14. Tajuk/Topik *</label><input className="input" value={topic} onChange={e=>setTopic(upper(e.target.value))}/></div>
        <div className="field"><label>15. Masa *</label><TimePicker value={pdpTime} onChange={setPdpTime}/></div>
      </div></div>
    </div>}
    ...
    {step===3&&instrument&&<div className="grid">
      ...
      <div className="card"><div className="official-section-title">Guru - Bahagian F - Refleksi (Diisi oleh pemantau semasa menemu bual guru sekolah)</div><div className="field"><label>1. Apa pandangan anda mengenai pengajaran dan pembelajaran (PdP) KBAT yang telah anda laksanakan tadi?</label><textarea rows={7} value={reflection1} onChange={e=>setReflection1(e.target.value)}/></div><div className="field" style={{marginTop:14}}><label>2. Bagaimana anda boleh membuat penambahbaikan terhadap PdP KBAT anda? (Nyatakan perancangan anda.)</label><textarea rows={7} value={reflection2} onChange={e=>setReflection2(e.target.value)}/></div></div>
      <div className="card"><div className="official-section-title">Bahagian G - Rumusan</div><div className="field"><label>Rumusan Keseluruhan Pemantau:</label><textarea rows={5} value={observerSummary} onChange={e=>setObserverSummary(e.target.value)}/></div><div className="form-grid" style={{marginTop:14}}><div className="field"><label>Tandatangan Pemantau:</label><input className="input official-readonly" readOnly value="Diisi pada salinan PDF / cetakan"/></div><div className="field"><label>Tarikh:</label><input className="input official-readonly" readOnly value={observationDate}/></div></div></div>
    </div>}
