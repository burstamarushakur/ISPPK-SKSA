...
export default function Pic() {
  ...
  return <main className="container">
    ...
    {tab==='records' && <>
      ...
      <div className="table-wrap"><table><thead><tr><th>Tahun</th><th>Guru</th><th>Tarikh</th><th>Subjek / Kelas</th><th>Skor</th><th>Status</th><th>RPH</th><th>Google Form</th><th></th></tr></thead><tbody>
        {filtered.map(o=>{const t=teachers.find(x=>x.id===o.teacherId);const c=classes.find(x=>x.id===o.classId);const s=subjects.find(x=>x.id===o.subjectId);const iv=instruments.find(x=>x.id===o.instrumentVersionId);const sc=iv?scoreSummary(o,iv):null;return <tr key={o.id}><td><span className="version-chip">{o.instrumentYearSnapshot}</span></td><td><strong>{t?.name||o.teacherNameSnapshot||'Guru'}</strong><div className="helper">{o.topic}</div></td><td>{formatDateMY(o.observationDate)}</td><td>{s?.name||o.subjectNameSnapshot}<div className="helper">{c?`TAHUN ${c.year} - ${c.name}`:o.classNameSnapshot}</div></td><td><strong>{sc?`G ${sc.teacherTotal}/${sc.teacherMax}`:'—'}</strong><div className="helper">{sc?`M ${sc.student}/${sc.studentMax}`:''}</div></td><td><span className={`status status-${o.status}`}>{o.status==='verified'?'DISAHKAN':o.status==='submitted'?'MENUNGGU':'DRAF'}</span></td><td>{o.rphPath?<span title={o.rphFileName||'RPH'}>📎 ADA</span>:<span className="helper">—</span>}</td><td>{o.googleFormStatus==='sent'?'✅ SUDAH':'⏳ BELUM'}</td><td><div className="toolbar"><button className="btn btn-secondary" onClick={()=>navigate(`/pic/rekod/${o.id}`)}>Buka</button><button className="btn btn-delete-record" onClick={()=>removeRecord(o)}>Padam</button></div></td></tr>})}
...
function SchoolPanel({value}:{value:SchoolSettings,onSaved:()=>Promise<void>}) {
  return <div className="card"><h2>Profil Sekolah</h2><p className="helper">Profil tetap SK Sungai Abong. Maklumat ini ditanam terus dan digunakan pada Bahagian B borang Guru dan Murid.</p><div className="form-grid">
    <div className="field"><label>1. Nama Sekolah</label><input className="input official-readonly" readOnly value={value.schoolName}/></div><div className="field"><label>2. Alamat Sekolah</label><input className="input official-readonly" readOnly value={value.address}/></div>
    <div className="field"><label>3. No. Tel.</label><input className="input official-readonly" readOnly value={value.phone}/></div><div className="field"><label>4. No. Faks</label><input className="input official-readonly" readOnly value={value.fax}/></div>
    <div className="field span-2"><label>5. E-mel</label><input className="input official-readonly" readOnly value={value.officialEmail}/></div><div className="field"><label>6. Kod Sekolah</label><input className="input official-readonly" readOnly value={value.schoolCode}/></div><div className="field"><label>7. Gred Sekolah</label><input className="input official-readonly" readOnly value={value.grade}/></div>
    <div className="field"><label>8. Jenis Sekolah</label><input className="input official-readonly" readOnly value={value.schoolType}/></div><div className="field"><label>9. Lokasi Sekolah</label><input className="input official-readonly" readOnly value={value.location}/></div><div className="field"><label>10. PPD</label><input className="input official-readonly" readOnly value={value.ppd}/></div><div className="field"><label>11. Negeri</label><input className="input official-readonly" readOnly value={value.state}/></div><div className="field span-2"><label>12. Program Sekolah</label><input className="input official-readonly" readOnly value={value.schoolProgram}/></div>
  </div></div>
}
